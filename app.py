        ...
        if st.button("📥 Fetch Content") and url:
            try:
                ...
                article = (soup.find('article') or 
                          soup.find('main') or 
                          soup.find('div', class_=lambda x: x and any(k in str(x).lower() for k in ['article', 'content', 'post', 'story', 'body', 'entry'])))
                ...
                st.success("✅ Đã lấy nội dung sạch!")
                st.text_area("Nội dung sạch (có thể chỉnh)", st.session_state.text_content, height=350, key="preview")
